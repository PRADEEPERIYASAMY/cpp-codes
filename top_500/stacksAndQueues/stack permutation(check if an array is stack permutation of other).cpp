...
bool checkStackPermutation(int ip[],int op[],int n){
    ...
    for (int i = 0; i < n; i++) input.push(ip[i]);
    for (int i = 0; i < n; i++) output.push(op[i]);
    stack<int> tempStack;
    while(!input.empty()){
        int ele = input.front();input.pop();
        if(ele == output.front()){
            output.pop();
            while(!tempStack.empty()){
                if(tempStack.top() == output.front()) tempStack.pop(),output.pop();
                else break;
            }
